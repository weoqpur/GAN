...
train_set = Dataset(os.path.join(data_dir, 'pokemon', transform=transform))
...
for epoch in range(1, num_epoch + 1):
    ...
    for batch, data in enumerate(train_loader, 1):
        ...
        valid = Variable(torch.FloatTensor(data.size(0), 1).fill_(1.0), requires_grad=False)
        fake = Variable(torch.FloatTensor(data.size(0), 1).fill_(0.0), requires_grad=False)
        ...
        z = Variable(torch.FloatTensor(np.random.normal(0, 1, (data.shape[0], 100))))
        ...
        print(
            ...
            % (num_epoch, epoch, batch_size, batch, np.mean(loss_train), np.mean(loss_D_real_train),
